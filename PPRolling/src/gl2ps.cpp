...
#define GL2PS_MAGEMAP         7
#define GL2PS_MAGEMAP_WRITTEN 8
#define GL2PS_MAGEMAP_VISIBLE 9
...
#define GL2PS_DRAW_MAGEMAP_TOKEN      13
...
static void *gl2psMalloc(size_t size)
{
	...
	ptr = malloc(size);
	if (!ptr) {
		gl2psMsg(GL2PS_ERROR,"could not allocate requested memmory");
		exit(1);
	}
	return(ptr);
}
	...
	GLshort *index0, GLshort *index1)
{
	...
	if (parent->type == GL2PS_MAGEMAP) {
		child->type = GL2PS_MAGEMAP;
...
static void gl2psRescaleAndOffset()
{
	...
	for (i = 0; i < gl2psListNbr(gl2ps->primitives); i++) {
		...
		if ((gl2ps->options & GL2PS_SMPLE_LINE_OFFSET) &&
			(prim->type == GL2PS_LINE)) {
			if (gl2ps->sort == GL2PS_SMPLE_SORT) {
...
static GLint gl2psAddInBspImageTree(GL2PSprimitive *prim, GL2PSbsptree2d **tree)
{
	...
	if (*tree == NULL) {
		if ((prim->type != GL2PS_MAGEMAP) && (GL_FALSE == gl2ps->zerosurfacearea)) {
...
static void gl2psAddInImageTree(void *data)
{
	...
	if (prim->type == GL2PS_MAGEMAP && prim->data.image->format == GL2PS_MAGEMAP_VISIBLE) {
		prim->culled = 1;
	}
	else if (!gl2psAddInBspImageTree(prim, &gl2ps->imagetree)) {
		prim->culled = 1;
	}
	else if (prim->type == GL2PS_MAGEMAP) {
		prim->data.image->format = GL2PS_MAGEMAP_VISIBLE;
	}
}
	...
	GLfloat width, char boundary)
{
	...
	tris/quads to enhance SMPLE_SORT */
	...
	while (used > 0) {
		...
		case GL_PASS_THROUGH_TOKEN:
			...
			case GL2PS_SET_LINE_WIDTH:
				...
				lwidth = current[1];
				break;
			case GL2PS_DRAW_MAGEMAP_TOKEN:
				prim = (GL2PSprimitive *)gl2psMalloc(sizeof(GL2PSprimitive));
				prim->type = GL2PS_MAGEMAP;
				prim->boundary = 0;
				prim->numverts = 4;
...
static void gl2psPrintPostScriptPrimitive(void *data)
{
	...
	case GL2PS_PIXMAP:
		...
		break;
	case GL2PS_MAGEMAP:
		if (prim->data.image->type != GL2PS_MAGEMAP_WRITTEN) {
			...
			prim->data.image->type = GL2PS_MAGEMAP_WRITTEN;
		}
		break;
	case GL2PS_TEXT:
		gl2psPrintPostScriptColor(prim->verts[0].rgba);
...
static void gl2psPrintTeXHeader(void)
{
	...
	if (gl2ps->filename && strlen(gl2ps->filename) < 256) {
		for (i = (int)(strlen(gl2ps->filename) - 1); i >= 0; i--) {
			if (gl2ps->filename[i] == '.') {
				strncpy_s(name, gl2ps->filename, i);
				name[i] = '\0';
				break;
			}
		}
		if (i <= 0) strcpy_s(name, gl2ps->filename);
	}
	else {
		strcpy_s(name, "untitled");
	}
...
GL2PSDLL_API GLint gl2psDrawImageMap(GLsizei width, GLsizei height, const GLfloat position[3], const unsigned char *imagemap) {
	...
	size = height + height * ((width - 1) / 8);
	glPassThrough(GL2PS_DRAW_MAGEMAP_TOKEN);
	glBegin(GL_POINTS);
